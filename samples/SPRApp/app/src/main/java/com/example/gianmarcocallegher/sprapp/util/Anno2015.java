...
public class Anno2015 extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        for (SoldipubbliciParser.Data x : spese_ente)
            spesePROCapite.add(x.descrizione_codice + "\nSpesa Totale: " + (Double.parseDouble(x.importo_2015) / 100) +
                    "\nSpesa PRO-Capite: " + (Double.parseDouble(x.importo_2015) / 100) / Double.parseDouble(numero_abitanti)
            );
        ...
        inputSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                Anno2015.this.adapter.getFilter().filter(s);
            }
